...
<body>
	<div class="container">
		...
		value="${param.startDate}"/>&amp;endDate=<c:out value="${param.endDate}"/>" 
		...
		<form action="attendance" method="post" enctype="multipart/form-data">
			...
			<button type="submit" class="button">CSVインポート</button>
		</form>
			...
			<table>
				...
				<tbody>
					<c:forEach var="entry" items="${monthlyOvertimeHours}">
						<tr>
							<td>
								<c:set var="yearMonthString" value="${entry.key}" />
								<c:set var="year" value="${fn:substring(yearMonthString, 0, 4)}" />
								<c:set var="month" value="${fn:substring(yearMonthString, 5, 7)}" />
								${year}年${month}月
							</td>
							<td>${entry.value}</td>
						</tr>
		...
		<c:if test="${empty monthlyOvertimeHours}">
			<p>残業時間のデータがありません</p>
		</c:if>
