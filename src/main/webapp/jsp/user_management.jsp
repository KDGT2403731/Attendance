...
<body>
	<div class="container">
		...
		<form action="users" method="post" class="user-form">
			<input type="hidden" name="action" value="<c:choose><c:when 
			test="${userToEdit != null}">update</c:when><c:otherwise>add</c:otherwise></c:choose>">
			...
			<div class="button-group">
				...
				test="${userToEdit != null}">更新</c:when><c:otherwise>追加</c:otherwise></c:choose>">
				<c:if test="${userToEdit != null}">
					...
					<tr>
						...
						<td>
							<form action="users" method="post" style="display:inline;">
								...
								<input type="submit" value="<c:choose><c:when
test="${u.enabled}">無効化</c:when><c:otherwise>有効化</c:otherwise></c:choose>" class="button<c:choose><c:when
